...
from tradify.wallet.models import Balance
import time
...
class TransactionService:
    def __init__(self, session_factory:sessionmaker[Session]):  # Accept a session factory (e.g. sessionmaker)
        ...
        # CurrencyService and WalletService will also need sessions per method call now
        ...
        with self.session_factory() as session:
            ...
            for attempt in range(4):
                try:
                    ...
                    self.create_record(session=session, wallet_id=wallet.id, tx_type=TransactionType('fund'), amount=amount, from_code=currency.code, to_code=None)
                    return True
    ...
    def withdraw(self, user_id: UUID, currency_code: str, amount: float):
        with self.session_factory() as session:
            ...
            wallet = wallet_service.get_wallet(user_id)
            if not wallet:
                return {'success': False, 'code': 'W01'}

            currency = currency_service.currency_get(currency_code)
            if not currency:
                return {'success': False, 'code': 'W02'}

            balance = next((b for b in wallet.balances if b.currency_id == currency.id), None)
            if not balance:
                return {'success': False, 'code': 'W03'}
        ...
        with self.session_factory() as session:
            ...
            for attempt in range(4):
                try:
                    ...
                    self.create_record(session=session, wallet_id=wallet.id, tx_type=TransactionType('trae'), amount=amount, from_code=from_currency.code, to_code=to_currency.code)

                    return {'success': True, 'code': 'S00'}
